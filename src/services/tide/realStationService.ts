...
export async function fetchRealStationMetadata(): Promise<NoaaStationMetadata[]> {
  ...
  if (response.ok) {
    const data = (await response.json()) as { stations?: RawStation[] } | null;
  ...
  const responseBackup = await fetch('/stations.json');
  if (responseBackup.ok) {
    const data = (await responseBackup.json()) as { stations?: RawStation[] } | null;
